...
- **[MARATHON_CONTENT_UPDATE_GUIDE.md](MARATHON_CONTENT_UPDATE_GUIDE.md)** - 📝 Инструкция для редакторов: обновление описаний дней в марафонах

## 🛠️ Утилиты и скрипты

### Проверка и исправление марафонов

- **[src/scripts/fix-all-marathon-new-exercises.ts](src/scripts/fix-all-marathon-new-exercises.ts)** - Автоматическая проверка и исправление поля `newExerciseIds` во всех марафонах

  ```bash
  # Запуск скрипта
  npx tsx src/scripts/fix-all-marathon-new-exercises.ts
  ```
  
  Скрипт проверяет корректность пометки "новых упражнений" в каждом дне марафона и автоматически исправляет несоответствия.
