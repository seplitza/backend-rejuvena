...
async function updateProbabilities() {
  try {
    ...
    // Получаем все призы
    const prizes = await FortuneWheelPrize.find({}).sort({ displayOrder: 1 });
    ...
    for (const update of updates) {
      const prize = await FortuneWheelPrize.findOne({ name: update.name });
      if (prize) {
    ...
    const discountPrizes = await FortuneWheelPrize.find({ type: 'discount' });
    const shippingPrizes = await FortuneWheelPrize.find({ type: 'freeShipping' });
    ...
    const updatedPrizes = await FortuneWheelPrize.find({}).sort({ displayOrder: 1 });
    let newTotal = 0;
