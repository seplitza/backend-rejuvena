...
async function reorganizePrizes() {
  try {
    ...
    const prizes = await FortuneWheelPrize.find({ isActive: true }).sort({ _id: 1 });
    ...
    if (prizes.length >= 16) {
      ...
      const newPrize = new FortuneWheelPrize({
    ...
    // Reload all prizes
    const allPrizes = await FortuneWheelPrize.find({ isActive: true }).sort({ _id: 1 });
    ...
    for (let idx = 0; idx < orderedPrizes.length; idx++) {
      ...
      await FortuneWheelPrize.updateOne(
        { _id: prize._id },
    ...
    // Show final order
    const finalPrizes = await FortuneWheelPrize.find({ isActive: true }).sort({ displayOrder: 1 });
    console.log('\n📋 Final prize order:');
