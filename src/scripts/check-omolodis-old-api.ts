...
async function checkOmolodisOldAPI() {
  try {
    ...
    const marathonResponse = await axios.get(`${AZURE_API}/api/usermarathon/startmarathon`, {
      params: {
    ...
    if (marathon.marathonDays && marathon.marathonDays.length > 0) {
      marathon.marathonDays.forEach((day: any, index: number) => {
    ...
    if (marathon.greatExtensionDays && marathon.greatExtensionDays.length > 0) {
      marathon.greatExtensionDays.forEach((day: any, index: number) => {
