...
class DataImportParser {
     ...
     */
    static normalizeFields(data, type, userMapping = {}) {
        const fieldMappings = {
            orders: {
 ...
 */
router.post('/preview', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
 ...
 */
router.post('/execute', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'Файл не загружен' });
        }
        ...
        if (dataType === 'users') {
            ...
            for (const userData of normalizedData) {
                try {
                    ...
                    if (hasRealEmail) {
                        existingUser = await User_model_1.default.findOne({ email: userEmail });
                    }
                    if (!existingUser && userPhone) {
                        existingUser = await User_model_1.default.findOne({ phone: userPhone });
                    }
                    if (existingUser) {
                        if (mode === 'insert') {
                            skipped++;
            ...
            for (const orderData of normalizedData) {
                try {
                    ...
                    if (!user && userPhone) {
                        user = await User_model_1.default.findOne({ phone: userPhone });
                    }
                    if (!user) {
                        // Извлекаем ФИО из данных
                        let firstName = '';
                    ...
                    if (mode === 'upsert' && existing) {
                        await Order_model_1.default.updateOne({ _id: existing._id }, orderDoc);
                        updated++;
                    }
                    else if (!existing) {
                        await Order_model_1.default.create(orderDoc);
                        imported++;
                    }
                    else {
            ...
            for (const productData of normalizedData) {
                try {
                    ...
                    const existing = await Product_model_1.default.findOne({ sku });
                    if (existing) {
                        if (mode === 'insert') {
                            skipped++;
                            continue;
                    ...
                    if (productData.category) {
                        ...
                        const categorySlug = categoryName
                            ...
                            .replace(/\s+/g, '-');
                        let category = await ProductCategory_model_1.default.findOne({
                            $or: [
                                { name: categoryName },
                                { slug: categorySlug }
                            ]
                    ...
                    if (mode === 'upsert' && existing) {
                        await Product_model_1.default.updateOne({ _id: existing._id }, productDoc);
                        updated++;
                    }
                    else if (!existing) {
                        await Product_model_1.default.create(productDoc);
                        imported++;
                    }
                    else {
                        skipped++;
 ...
 * История импортов
 */
router.get('/history', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
        // Находим все импортированные заказы
        const importedOrders = await Order_model_1.default.find({
            orderNumber: /^(CRM-|IMPORT-)/
        }).sort({ createdAt: -1 }).limit(100);
        const stats = {
            totalImportedOrders: importedOrders.length,
            crmOrders: await Order_model_1.default.countDocuments({ orderNumber: /^CRM-/ }),
            customImports: await Order_model_1.default.countDocuments({ orderNumber: /^IMPORT-/ })
        };
        res.json({
            success: true,
            data: {
