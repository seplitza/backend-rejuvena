...
router.get('/products/:id', async (req, res) => {
    try {
        ...
        const [wbPrice, ozonPrice] = await Promise.all([
            product.articleWB
                ? MarketplacePrice_model_1.default.findOne({
                    product: product._id,
                    marketplace: 'wildberries'
        ...
        let maxMarketplacePrice = 0;
        if (wbPrice?.price)
            maxMarketplacePrice = Math.max(maxMarketplacePrice, wbPrice.price);
        if (ozonPrice?.price)
            maxMarketplacePrice = Math.max(maxMarketplacePrice, ozonPrice.price);
        const savings = maxMarketplacePrice > product.price
            ? maxMarketplacePrice - product.price
            : 0;
        res.json({
...
router.get('/categories', async (req, res) => {
    try {
        const categories = await ProductCategory_model_1.default.find({ isActive: true })
...
router.post('/validate-promo', async (req, res) => {
    try {
        ...
        const promo = await PromoCode_model_1.default.findOne({
        ...
        if (promo.applicableProducts && promo.applicableProducts.length > 0) {
            const hasApplicableProducts = productIds.some((id) => promo.applicableProducts?.some(p => p.toString() === id));
 ...
 */
router.post('/checkout', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: 'Требуется авторизация' });
        }
        const { items, // [{ product, quantity, price }]
        shippingAddress, deliveryMethod, // 'courier' | 'pickup' | 'cdek'
        cdekOfficeCode, // if deliveryMethod === 'cdek'
        paymentMethod, // 'online' | 'cash' | 'card'
        promoCode, usePersonalDiscount, fortuneWheelGiftIds, // Apply won gifts
        ...
        for (const item of items) {
            const product = await Product_model_1.default.findOne({ _id: item.product, isActive: true });
            if (!product) {
        ...
        if (promoCode) {
            promoCodeDoc = await PromoCode_model_1.default.findOne({
        ...
        if (fortuneWheelGiftIds && fortuneWheelGiftIds.length > 0) {
            const gifts = user.fortuneWheelGifts?.filter(g => fortuneWheelGiftIds.includes(g._id.toString()) &&
 ...
 */
router.get('/orders', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: 'Требуется авторизация' });
        ...
        const [orders, total] = await Promise.all([
            Order_model_1.default.find({ user: userId })
                ...
                .lean(),
            Order_model_1.default.countDocuments({ user: userId })
        ]);
        res.json({
            orders,
            pagination: {
 ...
 */
router.get('/orders/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: 'Требуется авторизация' });
