...
router.get('/stats', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const [pending, urgent, needsResponse] = await Promise.all([
            ...
            Comment_model_1.default.countDocuments({
                status: 'pending',
...
router.get('/', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { status, priority, needsResponse, exerciseId, marathonId, userId, search, view = 'moderation', // 'moderation' or 'admin-replies'
        sortBy = 'createdAt', sortOrder = 'desc', page = 1, limit = 50 } = req.query;
        const filter = {};
        ...
        const comments = await Comment_model_1.default.find(filter)
            ...
            .limit(Number(limit))
            .lean();
        const total = await Comment_model_1.default.countDocuments(filter);
        // Get statistics
...
router.get('/:id/thread', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Get all replies
        const replies = await Comment_model_1.default.find({ parentCommentId: id })
            .populate('userId', 'email firstName lastName')
            .populate('respondedBy', 'firstName lastName email')
            .sort({ createdAt: 1 })
            .lean();
        ...
        if (comment.parentCommentId) {
            parentComment = await Comment_model_1.default.findById(comment.parentCommentId)
                .populate('userId', 'email firstName lastName')
                .lean();
        }
        res.json({
...
router.post('/:id/respond', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const originalComment = await Comment_model_1.default.findById(id);
        if (!originalComment) {
            return res.status(404).json({ success: false, error: 'Comment not found' });
        }
        // Create response comment
        const responseComment = new Comment_model_1.default({
            userId: req.userId,
            ...
            respondedBy: req.userId // Mark this as admin response
        });
        await responseComment.save();
        // Update original comment
        originalComment.adminResponseId = responseComment._id;
        originalComment.respondedBy = req.userId;
        originalComment.respondedAt = new Date();
        await originalComment.save();
        const populatedResponse = await Comment_model_1.default.findById(responseComment._id)
...
router.put('/:id/status', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const comment = await Comment_model_1.default.findByIdAndUpdate(id, { status }, { new: true }).populate('userId', 'email firstName lastName');
        if (!comment) {
            return res.status(404).json({ success: false, error: 'Comment not found' });
        }
...
router.put('/:id/priority', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const comment = await Comment_model_1.default.findByIdAndUpdate(id, { priority }, { new: true }).populate('userId', 'email firstName lastName');
        if (!comment) {
            return res.status(404).json({ success: false, error: 'Comment not found' });
        }
        res.json({ success: true, data: comment });
...
router.delete('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        // Delete all replies
        await Comment_model_1.default.deleteMany({ parentCommentId: id });
...
router.post('/:id/ai-suggest', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const comment = await Comment_model_1.default.findById(id)
            .populate('userId', 'email firstName lastName')
...
router.put('/:id/starred', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { starred } = req.body;
        const comment = await Comment_model_1.default.findByIdAndUpdate(id, { starred: starred !== undefined ? starred : true }, { new: true }).populate('userId', 'email firstName lastName');
        if (!comment) {
            return res.status(404).json({ success: false, error: 'Comment not found' });
        }
        res.json({ success: true, data: comment });
...
router.get('/exercises/list', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const exercises = await Comment_model_1.default.aggregate([
            {
                $match: {
