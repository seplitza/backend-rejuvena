...
router.post('/resend', async (req, res) => {
    try {
        ...
        const emailLog = await EmailLog_model_1.default.findOne({ providerId });
        if (!emailLog) {
        ...
        switch (type) {
            ...
            case 'email.delivered':
                ...
                if (emailLog.campaignId) {
                    await EmailCampaign_model_1.default.updateOne({ _id: emailLog.campaignId }, { $inc: { 'stats.delivered': 1 } });
                }
                break;
            case 'email.opened':
                if (!emailLog.openedAt) {
                    ...
                    if (emailLog.campaignId) {
                        await EmailCampaign_model_1.default.updateOne({ _id: emailLog.campaignId }, { $inc: { 'stats.opened': 1 } });
                    }
                }
                break;
            case 'email.clicked':
                if (!emailLog.clickedAt) {
                    ...
                    if (emailLog.campaignId) {
                        await EmailCampaign_model_1.default.updateOne({ _id: emailLog.campaignId }, { $inc: { 'stats.clicked': 1 } });
                    }
                }
                break;
            case 'email.bounced':
                ...
                // Update campaign stats
                if (emailLog.campaignId) {
                    await EmailCampaign_model_1.default.updateOne({ _id: emailLog.campaignId }, { $inc: { 'stats.bounced': 1 } });
                }
                break;
            case 'email.complained':
                ...
                if (emailLog.campaignId) {
                    await EmailCampaign_model_1.default.updateOne({ _id: emailLog.campaignId }, { $inc: { 'stats.unsubscribed': 1 } });
                }
                break;
            default:
