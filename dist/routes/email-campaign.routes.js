...
router.get('/stats', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const [active, scheduled, draft] = await Promise.all([
            EmailCampaign_model_1.default.countDocuments({ isActive: true }),
            EmailCampaign_model_1.default.countDocuments({ isActive: false, 'steps.0': { $exists: true } }),
            EmailCampaign_model_1.default.countDocuments({ isActive: false, steps: { $size: 0 } })
        ]);
        res.json({
            success: true,
            active,
...
router.get('/', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const campaigns = await EmailCampaign_model_1.default.find()
...
router.get('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const campaign = await EmailCampaign_model_1.default.findById(req.params.id)
            .populate('trigger.marathonId', 'title')
...
router.post('/', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        await campaign.save();
        const populated = await EmailCampaign_model_1.default.findById(campaign._id)
            .populate('trigger.marathonId', 'title')
            .populate('steps.templateId', 'name subject type');
...
router.put('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { name, description, trigger, steps, isActive } = req.body;
        const campaign = await EmailCampaign_model_1.default.findByIdAndUpdate(req.params.id, { name, description, trigger, steps, isActive }, { new: true })
            .populate('trigger.marathonId', 'title')
            .populate('steps.templateId', 'name subject type');
        if (!campaign) {
            return res.status(404).json({ success: false, error: 'Campaign not found' });
...
router.delete('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const campaign = await EmailCampaign_model_1.default.findByIdAndDelete(req.params.id);
        if (!campaign) {
            return res.status(404).json({ success: false, error: 'Campaign not found' });
        }
...
router.get('/:id/analytics', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const campaign = await EmailCampaign_model_1.default.findById(req.params.id);
        if (!campaign) {
            return res.status(404).json({ success: false, error: 'Campaign not found' });
        ...
        const logs = await EmailLog_model_1.default.find({ campaignId: campaign._id })
            .sort({ sentAt: -1 })
            .limit(100);
...
// Duplicate campaign
router.post('/:id/duplicate', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const original = await EmailCampaign_model_1.default.findById(req.params.id);
        if (!original) {
            return res.status(404).json({ success: false, error: 'Campaign not found' });
        }
        const duplicate = new EmailCampaign_model_1.default({
        ...
        const populated = await EmailCampaign_model_1.default.findById(duplicate._id)
            .populate('trigger.marathonId', 'title')
            .populate('steps.templateId', 'name subject type');
        res.json({ success: true, campaign: populated });
    }
...
router.get('/meta/triggers', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const marathons = await Marathon_model_1.default.find({ isPublic: true })
