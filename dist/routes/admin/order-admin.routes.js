...
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 20, status, paymentStatus, deliveryMethod, search, dateFrom, dateTo } = req.query;
        const query = {};
        if (status) {
            query.status = status;
        }
...
router.put('/:id/cdek', async (req, res) => {
    try {
        ...
        if (cdekOrderId)
            order.cdekOrderId = cdekOrderId;
        if (cdekBarcode)
            order.cdekBarcode = cdekBarcode;
        if (cdekTrackingNumber)
            order.cdekTrackingNumber = cdekTrackingNumber;
        if (cdekOfficeCode)
            order.cdekOfficeCode = cdekOfficeCode;
        await order.save();
        res.json({
            order,
...
router.get('/stats/summary', async (req, res) => {
    try {
        ...
        const [totalOrders, pendingOrders, completedOrders, cancelledOrders, totalRevenue, averageOrderValue] = await Promise.all([
            Order_model_1.default.countDocuments(query),
