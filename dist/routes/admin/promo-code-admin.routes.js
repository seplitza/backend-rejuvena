...
router.get('/', async (req, res) => {
    try {
        ...
        const [promoCodes, total] = await Promise.all([
            PromoCode_model_1.default.find(query)
                ...
                .limit(Number(limit))
                .lean(),
            PromoCode_model_1.default.countDocuments(query)
        ]);
        res.json({
            promoCodes,
            pagination: {
...
router.get('/:id', async (req, res) => {
    try {
        const promoCode = await PromoCode_model_1.default.findById(req.params.id)
...
router.post('/', async (req, res) => {
    try {
        const { code, description, discountType, discountValue, freeShipping, minOrderAmount, usageLimit, validUntil, applicableProducts, applicableCategories, isActive } = req.body;
        ...
        const existingPromo = await PromoCode_model_1.default.findOne({ code: code.toUpperCase() });
        if (existingPromo) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (minOrderAmount !== undefined)
            promoCode.minOrderAmount = minOrderAmount;
...
router.get('/:id/stats', async (req, res) => {
    try {
        const promoCode = await PromoCode_model_1.default.findById(req.params.id).lean();
        if (!promoCode) {
            return res.status(404).json({ error: 'Промокод не найден' });
        }
...
router.post('/generate', async (req, res) => {
    try {
        const { count = 10, prefix = '', discountType, discountValue, freeShipping, minOrderAmount, usageLimit = 1, validUntil, description } = req.body;
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Check uniqueness
            const exists = await PromoCode_model_1.default.findOne({ code });
            if (exists) {
