...
router.get('/', async (req, res) => {
    try {
        ...
        const categories = await ProductCategory_model_1.default.find(query)
...
router.get('/:id', async (req, res) => {
    try {
        const category = await ProductCategory_model_1.default.findById(req.params.id)
            .populate('parentCategory', 'name slug')
            .lean();
        if (!category) {
        ...
        // Get subcategories
        const subcategories = await ProductCategory_model_1.default.find({
...
router.post('/', async (req, res) => {
    try {
        const { name, slug, description, parentCategory, imageUrl, sortOrder, seo, isActive } = req.body;
        // Validate required fields
        if (!name || !slug) {
            return res.status(400).json({ error: 'Обязательные поля: название, slug' });
        }
        // Check slug uniqueness
        const existingCategory = await ProductCategory_model_1.default.findOne({ slug });
        if (existingCategory) {
            return res.status(400).json({ error: 'Категория с таким slug уже существует' });
        }
        // Validate parent category
        if (parentCategory) {
            const parent = await ProductCategory_model_1.default.findOne({ _id: parentCategory, isActive: true });
            if (!parent) {
                return res.status(400).json({ error: 'Родительская категория не найдена' });
            }
        }
        // Create category
        const category = new ProductCategory_model_1.default({
            name,
            slug,
            description,
            parentCategory,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const subcategoryCount = await ProductCategory_model_1.default.countDocuments({
            parentCategory: category._id,
            isActive: true
        });
...
router.put('/reorder', async (req, res) => {
    try {
        ...
        const updatePromises = categories.map(({ id, sortOrder }) => ProductCategory_model_1.default.findByIdAndUpdate(id, { sortOrder }));
