...
router.post('/grant-spins-test-users', async (req, res) => {
    try {
        const User = require('../../models/User.model').default;
        const result = await User.updateMany({ email: { $regex: 'test', $options: 'i' } }, { $set: { fortuneWheelSpins: 5, fortuneWheelGifts: [] } });
...
router.post('/recreate-prizes', async (req, res) => {
    try {
        // Delete old prizes
        const deleted = await FortuneWheelPrize_model_1.default.deleteMany({});
        console.log(`🗑️  Deleted ${deleted.deletedCount} old prizes`);
        // Get products for prizes
        const products = await Product_model_1.default.find({ isActive: true }).sort({ price: -1 }).limit(10);
        const prizes = [
        ...
        const created = await FortuneWheelPrize_model_1.default.insertMany(prizes);
        res.json({
            success: true,
            message: `✅ Создано ${created.length} новых призов (включая extraSpin)`,
            oldCount: deleted.deletedCount,
            newCount: created.length,
            distribution: created.map((p) => ({ name: p.name, type: p.type, probability: p.probability + '%' }))
        });
    }
    catch (error) {
 ...
 * Requires: admin auth
 */
router.post('/seed-prizes', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
        console.log('🎡 Создание призов для колеса фортуны...');
        // Проверяем, есть ли уже призы
        const existingCount = await FortuneWheelPrize_model_1.default.countDocuments({});
        if (existingCount > 0) {
 ...
 * Requires: admin auth
 */
router.delete('/delete-all-prizes', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
        const result = await FortuneWheelPrize_model_1.default.deleteMany({});
 ...
 */
router.get('/stats', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
        const prizes = await FortuneWheelPrize_model_1.default.find({}).sort({ probability: -1 });
        const totalProbability = prizes.reduce((sum, p) => sum + p.probability, 0);
        res.json({
 ...
 */
router.get('/winners', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
...
router.get('/settings', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
...
router.put('/settings', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
        ...
        let settings = await FortuneWheelSettings_model_1.default.findOne();
        if (!settings) {
            settings = await FortuneWheelSettings_model_1.default.create({
                isEnabled,
                updatedBy: userId
            });
        }
...
router.put('/prizes/:prizeId', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
        ...
        const prize = await FortuneWheelPrize_model_1.default.findByIdAndUpdate(prizeId, { probability }, { new: true });
        if (!prize) {
...
router.put('/winners/:spinId/toggle-status', [authMiddleware_1.authMiddleware, authMiddleware_1.adminMiddleware], async (req, res) => {
    try {
        const WheelSpin = require('../../models/WheelSpin.model').default;
