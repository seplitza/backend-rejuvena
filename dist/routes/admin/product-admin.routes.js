...
router.get('/:id', async (req, res) => {
    try {
        ...
        const priceHistory = await MarketplacePrice_model_1.default.find({
            product: product._id
        })
            .sort({ fetchedAt: -1 })
            .limit(100)
...
router.post('/', async (req, res) => {
    try {
        const { sku, name, slug, description, shortDescription, price, compareAtPrice, stock, category, images, isActive, isFeatured, 
        // General info
        brand, manufacturer, countryOfOrigin, barcode, vendorCode, weight, dimensions, characteristics, 
        // Marketplaces
        wildberries, ozon, yandexMarket, avito, 
        // SEO and metadata
        metadata, seo, tags } = req.body;
        ...
        const existingProduct = await Product_model_1.default.findOne({ sku });
        if (existingProduct) {
            return res.status(400).json({ error: 'Товар с таким артикулом уже существует' });
        }
        // Check slug uniqueness
        const existingSlug = await Product_model_1.default.findOne({ slug });
        if (existingSlug) {
            return res.status(400).json({ error: 'Товар с таким slug уже существует' });
        }
        // Validate category if provided
        if (category) {
            const categoryDoc = await ProductCategory_model_1.default.findById(category);
            if (!categoryDoc) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { sku, name, slug, description, shortDescription, price, compareAtPrice, stock, category, images, isActive, isFeatured, brand, manufacturer, countryOfOrigin, barcode, vendorCode, weight, dimensions, characteristics, wildberries, ozon, yandexMarket, avito, metadata, seo, tags } = req.body;
        ...
        if (wildberries !== undefined)
            product.wildberries = wildberries;
        if (ozon !== undefined)
            product.ozon = ozon;
        if (yandexMarket !== undefined)
            product.yandexMarket = yandexMarket;
        if (avito !== undefined)
            product.avito = avito;
...
router.post('/bulk-update', async (req, res) => {
    try {
        ...
        const result = await Product_model_1.default.updateMany({ _id: { $in: productIds } }, { $set: updates });
        res.json({
...
router.get('/:id/marketplace-prices', async (req, res) => {
    try {
        ...
        const prices = await MarketplacePrice_model_1.default.find({
