...
const alfabank_service_1 = __importDefault(require("../services/alfabank.service"));
...
async function generateOrderNumber() {
    const paymentCount = await Payment_model_1.default.countDocuments();
 ...
 */
router.post('/create-exercise', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const existingPurchase = await ExercisePurchase_model_1.default.findOne({
            userId,
            exerciseId,
 ...
 */
router.post('/create-marathon', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const marathon = await Marathon_model_1.default.findById(marathonId);
        if (!marathon) {
            return res.status(404).json({ error: 'Marathon not found' });
        }
        // Генерируем уникальный номер заказа
 ...
 */
router.get('/status/:paymentId', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { paymentId } = req.params;
        // Try to find by MongoDB _id first, then by alfaBankOrderId
        let payment = await Payment_model_1.default.findById(paymentId).catch(() => null);
        if (!payment) {
            payment = await Payment_model_1.default.findOne({ alfaBankOrderId: paymentId });
        }
        if (!payment) {
        ...
        if (payment.alfaBankOrderId) {
            try {
                const alfaStatus = await alfabank_service_1.default.getOrderStatus(payment.alfaBankOrderId);
                const newStatus = alfabank_service_1.default.getOrderStatusString(alfaStatus.orderStatus);
 ...
 */
router.get('/status-public/:orderId', async (req, res) => {
    try {
        const { orderId } = req.params;
        // Ищем платеж по orderId (alfaBankOrderId)
        let payment = await Payment_model_1.default.findOne({ alfaBankOrderId: orderId });
        // Если не нашли, пробуем найти по orderNumber (может быть передан вместо orderId)
        if (!payment) {
            payment = await Payment_model_1.default.findById(orderId).catch(() => null);
        }
        if (!payment) {
            return res.status(404).json({
                success: false,
                ...
                if (payment.status !== newStatus) {
                    ...
                    if (newStatus === 'succeeded') {
                        if (payment.metadata?.type === 'exercise' && payment.metadata.exerciseId && payment.metadata.exerciseName) {
                            ...
                            await activatePremium(payment.userId.toString(), payment.metadata?.planType, payment.metadata?.duration);
                        }
                    }
                    await payment.save();
                }
...
router.get('/history', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const payments = await Payment_model_1.default.find({ userId })
            .sort({ createdAt: -1 })
            .limit(Number(limit))
            .skip((Number(page) - 1) * Number(limit));
        const total = await Payment_model_1.default.countDocuments({ userId });
        return res.status(200).json({
            success: true,
...
async function handleWebhook(req, res) {
    try {
        ...
        // Проверяем актуальный статус в Альфа-Банке
        const alfaStatus = await alfabank_service_1.default.getOrderStatus(orderId || payment.alfaBankOrderId);
        const newStatus = alfabank_service_1.default.getOrderStatusString(alfaStatus.orderStatus);
        payment.status = newStatus;
        await payment.save();
...
router.get('/callback', async (req, res) => {
    try {
        ...
        const alfaStatus = await alfabank_service_1.default.getOrderStatus(orderId);
        const payment = await Payment_model_1.default.findOne({ alfaBankOrderId: orderId });
        if (!payment) {
 ...
 */
async function activatePremium(userId, planType, duration) {
    try {
 ...
 */
async function activateExercise(userId, exerciseId, exerciseName, price) {
    try {
        // Создаем запись о покупке упражнения
        const expiresAt = new Date();
        expiresAt.setMonth(expiresAt.getMonth() + 1); // +1 месяц доступа
        const purchase = new ExercisePurchase_model_1.default({
            userId,
            exerciseId,
            exerciseName,
            price,
...
async function activateMarathon(userId, marathonId, paymentId) {
    try {
        ...
        let enrollment = await MarathonEnrollment_model_1.default.findOne({ userId, marathonId });
        ...
        if (enrollment) {
            ...
            enrollment = new MarathonEnrollment_model_1.default({
                userId,
                marathonId,
                status: 'active',
                isPaid: true,
        ...
        if (marathonId === ADVANCED_MARATHON_ID) {
            ...
            let basicEnrollment = await MarathonEnrollment_model_1.default.findOne({
                userId,
                marathonId: BASIC_MARATHON_ID
            });
            const basicMarathon = await Marathon_model_1.default.findById(BASIC_MARATHON_ID);
            if (!basicMarathon) {
                ...
                if (basicEnrollment) {
                    ...
                    basicEnrollment = new MarathonEnrollment_model_1.default({
                        userId,
                        marathonId: BASIC_MARATHON_ID,
                        status: 'active',
                        isPaid: true,
                ...
                if (user?.email) {
                    try {
                        await email_service_1.default.sendMarathonEnrollmentEmail(user.email, basicMarathon.title, basicMarathon.startDate, true, // paid as part of advanced purchase
        ...
        try {
            ...
            if (user?.email && marathon) {
                await email_service_1.default.sendMarathonEnrollmentEmail(user.email, marathon.title, marathon.startDate, true, // paid marathon
...
router.get('/admin/all', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const payments = await Payment_model_1.default.find(filter)
            .populate('userId', 'email firstName lastName isPremium premiumEndDate createdAt')
            .sort({ createdAt: -1 })
            .limit(Number(limit))
            .skip(skip)
            .lean();
        const total = await Payment_model_1.default.countDocuments(filter);
        // Форматируем ответ
 ...
 */
router.patch('/admin/:paymentId/status', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        // Проверка прав администратора
        if (req.userRole !== 'superadmin' && req.userRole !== 'admin') {
            return res.status(403).json({ error: 'Access denied' });
        ...
        const payment = await Payment_model_1.default.findByIdAndUpdate(paymentId, { status }, { new: true }).populate('userId', 'email firstName lastName');
        if (!payment) {
            return res.status(404).json({ error: 'Payment not found' });
        }
        ...
        if (status === 'succeeded') {
            ...
            if (payment.metadata?.type === 'exercise' && payment.metadata.exerciseId && payment.metadata.exerciseName) {
                await activateExercise(userId, payment.metadata.exerciseId, payment.metadata.exerciseName, payment.amount / 100);
            }
            // Активация премиума
            ...
            // Активация марафона
                ...
                await activateMarathon(userId, payment.metadata.marathonId, payment._id.toString());
