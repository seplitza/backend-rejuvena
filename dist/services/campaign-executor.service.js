...
class CampaignExecutor {
    ...
    async processCampaigns() {
        ...
        try {
            const activeCampaigns = await EmailCampaign_model_1.default.find({ isActive: true });
     ...
     */
    async getUsersForTrigger(campaign) {
     ...
     */
    async getUsersForMarathonEnrollment(marathonId) {
     ...
     */
    async getUsersForMarathonStart(marathonId) {
        ...
        const enrollments = await MarathonEnrollment_model_1.default.find({ marathonId }).populate('userId');
        return enrollments
            .map((e) => e.userId)
            .filter((u) => u && u.email);
    }
     ...
     */
    async getUsersForMarathonDay(marathonId, dayNumber) {
        const marathon = await Marathon_model_1.default.findById(marathonId);
        if (!marathon)
            return [];
     ...
     */
    async getUsersForMarathonCompletion(marathonId) {
        const marathon = await Marathon_model_1.default.findById(marathonId);
        if (!marathon)
            return [];
     ...
     */
    async getUsersForPremiumPurchase() {
        const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);
     ...
     */
    async processUserCampaign(campaign, user) {
        // Get all logs for this user in this campaign
        const logs = await EmailLog_model_1.default.find({
        ...
        const lastStepIndex = campaign.steps.findIndex((s) => s.id === lastLog.stepId);
     ...
     */
    async checkCondition(condition, logs) {
     ...
     */
    async sendCampaignStep(campaign, step, user, previousLog) {
        try {
            // Get template
            const template = await EmailTemplate_model_1.default.findById(step.templateId);
            ...
            // Log the send
            const emailLog = new EmailLog_model_1.default({
                campaignId: campaign._id,
                stepId: step.id,
            ...
            await EmailCampaign_model_1.default.updateOne({ _id: campaign._id }, { $inc: { 'stats.sent': 1 } });
     ...
     */
    async buildTemplateData(campaign, user) {
        const data = {
        ...
        if (campaign.trigger.marathonId) {
            const marathon = await Marathon_model_1.default.findById(campaign.trigger.marathonId);
            if (marathon) {
