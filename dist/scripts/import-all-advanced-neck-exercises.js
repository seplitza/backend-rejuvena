...
async function importAllExercises() {
    try {
        ...
        for (const category of COURSE_DATA.dayCategories) {
            ...
            const categoryTags = await Promise.all(categoryTagNames.map(async (name) => {
                const slug = name.toLowerCase().replace(/\s+/g, '-');
                let tag = await Tag_model_1.default.findOne({ slug });
                if (!tag) {
            ...
            for (const oldExercise of category.exercises) {
                ...
                try {
                    ...
                    for (const ruEx of ruExercises) {
                        ...
                        const commonTags = enExTags.filter(tag => ruExTags.includes(tag) &&
