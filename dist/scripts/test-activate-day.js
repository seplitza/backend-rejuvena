...
async function test() {
    ...
    try {
        const enrollRes = await axios_1.default.post(`${AZURE_API}/api/usermarathon/enrollmarathon`, { marathonId }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    try {
        const activateRes = await axios_1.default.post(`${AZURE_API}/api/usermarathon/activateday`, { dayId, marathonId }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    try {
        const dayRes = await axios_1.default.get(`${AZURE_API}/api/usermarathon/getdayexercise?dayId=${dayId}`, { headers: { Authorization: `Bearer ${token}` } });
        console.log('✅ Success!');
