...
async function addPracticeDays() {
    try {
        ...
        const existingDays = await MarathonDay_model_1.default.find({
            marathonId: MARATHON_ID,
            dayNumber: { $gte: 16, $lte: 30 }
        });
        if (existingDays.length > 0) {
            console.log(`\n⚠️  Found ${existingDays.length} existing practice days. Deleting them first...`);
            await MarathonDay_model_1.default.deleteMany({
                marathonId: MARATHON_ID,
                dayNumber: { $gte: 16, $lte: 30 }
            });
        ...
        const created = await MarathonDay_model_1.default.insertMany(daysToCreate);
        ...
        // Get total count
        const totalDays = await MarathonDay_model_1.default.countDocuments({ marathonId: MARATHON_ID });
