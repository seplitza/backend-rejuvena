...
async function listAzureMarathons() {
    try {
        ...
        marathons.forEach((marathon, index) => {
        ...
        marathons
            .filter((m) => m.isDisplay && m.days > 0) // Only displayed marathons with days
            .forEach((marathon) => {
        ...
        const exampleMarathon = await axios_1.default.get(`${AZURE_API}/api/usermarathon/startmarathon?marathonId=3842e63f-b125-447d-94a1-b1c93be38b4e`, {
            headers: { Authorization: `Bearer ${token}` },
        });
        ...
        if (data.marathonDays) {
            data.marathonDays.forEach((day, index) => {
